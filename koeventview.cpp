...
QMenu *KOEventView::newEventPopup()
{
  ...
  if ( !client ) {
    kError() <<"KOEventView::newEventPopup(): no xmlGuiClient.";
    return 0;
  }
  if ( !client->factory() ) {
    kError() <<"KOEventView::newEventPopup(): no factory";
...
{
  kDebug(5850) <<"KOEventView::defaultAction()";

  if ( !incidence ) return;

  kDebug(5850) <<"  type:" << incidence->type();
