...
void KOViewManager::viewActionEnable(QAction *action)
{
    ...
    /* For agenda view range selections we change the top-level agenda view button text */
    if (isAgendaViewAction(action, ac)) {
        QAction *agendaAction = ac->action(QStringLiteral("view_agenda"));
        agendaAction->setText(i18n("Agenda: %1", action->text()));
        agendaAction->setStatusTip(action->statusTip());
        agendaAction->setToolTip(action->toolTip());
        agendaAction->setWhatsThis(action->whatsThis());
    }
